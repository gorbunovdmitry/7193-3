...
function renderLanding() {
  ...
  if (!sessionStorage.getItem('landingViewed')) {
    sendAnalyticsEvent('7193_page_view_var3', '7193_page_view_var3');
    sessionStorage.setItem('landingViewed', '1');
  }
  // Если уже была заглушка для этого варианта, не показываем лендинг
  if (localStorage.getItem('7193_placeholderShown_var3') === '1') {
    renderPlaceholder();
    return;
  }
  app.innerHTML = `
  ...
  document.getElementById('sendBtn').onclick = () => {
    sendAnalyticsEvent('7193_click_continue_var3', '7193_click_continue_var3');
    localStorage.setItem('7193_placeholderShown_var3', '1');
    renderPlaceholder();
...
function renderPlaceholder() {
  ...
  if (!sessionStorage.getItem('endPageViewed')) {
    sendAnalyticsEvent('7193_end_page_view_var3', '7193_end_page_view_var3');
